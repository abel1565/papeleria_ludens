...
@forelse ($cart->items ?? [] as $item)
  @if($item->productos)
    ...
    <article class="flex flex-col sm:flex-row items-center sm:items-start gap-6 border-b border-gray-200 pb-6 mb-6">
      <!-- Imagen del producto -->
      <div class="bg-gray-100 flex justify-center items-center w-40 h-40 sm:w-48 sm:h-48 rounded-lg overflow-hidden">
        <img src="{{ asset($producto->image ?? 'https://via.placeholder.com/150') }}"
             alt="{{ $producto->name }}"
      ...
      <div class="flex-1 w-full">
        <div class="flex justify-between items-start">
          <div>
            <h3 class="font-semibold text-lg text-gray-900">{{ $producto->name }}</h3>
            <p class="text-gray-500 text-sm mt-1">{{ ucfirst($categoria) }}</p>

            @if($colorNombres)
              <p class="text-gray-500 text-sm mt-1">{{ $colorNombres }}</p>
            @endif
          </div>
...
@empty
  <div class="text-center py-10 border-b border-gray-200">
    <p class="text-xl text-gray-700">Tu bolsa de compra está vacía.</p>
    <a href="{{ route('dashboard') }}" class="mt-4 inline-block font-semibold text-pink-600 hover:underline">Seguir comprando</a>
  </div>
@endforelse
